...
swift rlhf --rlhf_type grpo \
    ...
    --model $DIST_MODEL \
    --reward_funcs $DIST_REWARD_FUNCS \
    ${DIST_REWARD_PLUGINS:+--external_plugins "$DIST_REWARD_PLUGINS"} \
    --reward_weights $DIST_REWARD_WEIGHTS \
    --use_vllm true \
    --vllm_mode colocate \
    --vllm_gpu_memory_utilization 0.2 \
    --vllm_tensor_parallel_size $NPROC_PER_NODE \
    --vllm_max_model_len $((DIST_MAX_SEQ_LEN + DIST_MAX_GEN_LEN)) \
    --torch_dtype bfloat16 \
    --dataset $TRAIN_DATASET \
    ${VAL_DATASET:+--val_dataset} \
    ${VAL_DATASET:---split_dataset_ratio $DIST_DATASET_SPLIT} \
    --max_completion_length $DIST_MAX_GEN_LEN \
