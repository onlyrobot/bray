...
class AtariModel(torch.nn.Module):
    def __init__(self, action_space=6):
        ...
        self.base_net = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Conv2d(
                in_channels=32, out_channels=256, kernel_size=11, stride=1, padding=0
            ),
            torch.nn.ReLU(),
            torch.nn.Flatten(),
        )
        self.values_net = torch.nn.Linear(in_features=256, out_features=1)
        self.logits_net = torch.nn.Linear(in_features=256, out_features=action_space)
